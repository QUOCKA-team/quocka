...
for t in vislist:
	...
	call(['invert', 'vis=%s'%t_pscal, 'map=%s'%t_map, 'beam=%s'%t_beam, 'robust=0.5', 'stokes=i', 'options=mfs,double,sdb', 'imsize=2,2,beam', 'cell=5,5,res'],stdout=logf,stderr=logf)
	...
	call(['mfclean', 'map=%s'%t_map, 'beam=%s'%t_beam, 'out=%s'%t_model, 'niters=10000', 'cutoff=%s,%s'%(clean_level, 2*sigma), "region='perc(90)'"],stdout=logf,stderr=logf)
	...
	call(['mfclean', 'map=%s'%t_map, 'beam=%s'%t_beam, 'out=%s'%t_model, 'niters=1500', 'cutoff=%s,%s'%(clean_level, 2*sigma), 'region=mask(%s)'%t_mask],stdout=logf,stderr=logf)
	call(['selfcal', 'vis=%s'%t_pscal, 'model=%s'%t_model, 'interval=5', 'nfbin=1', 'options=phase,mfs'],stdout=logf,stderr=logf)
	shutil.rmtree(t_map)
	...
	call(['selfcal', 'vis=%s'%t_pscal, 'model=%s'%t_model, 'interval=0.5', 'nfbin=1', 'options=phase,mfs'],stdout=logf,stderr=logf)
	shutil.rmtree(t_map)
	shutil.rmtree(t_beam)
	shutil.rmtree(t_mask)
	shutil.rmtree(t_model)
	...
	call(['selfcal', 'vis=%s'%t_ascal, 'model=%s'%t_model, 'interval=5', 'nfbin=1', 'options=amp,mfs'],stdout=logf,stderr=logf)
	shutil.rmtree(t_map)
	shutil.rmtree(t_beam)
	shutil.rmtree(t_mask)
	shutil.rmtree(t_model)
	...
	t_p2a1 = t + '.p2a1.fits'	
	call(['invert', 'vis=%s'%t_ascal, 'map=%s'%t_map, 'beam=%s'%t_beam, 'robust=0.5', 'stokes=i', 'options=mfs,double,sdb', 'imsize=2,2,beam', 'cell=5,5,res'],stdout=logf,stderr=logf)
	call(['mfclean', 'map=%s'%t_map, 'beam=%s'%t_beam, 'out=%s'%t_model, 'niters=10000', 'cutoff=%s,%s'%(clean_level,2*sigma), "region='perc(90)'"],stdout=logf,stderr=logf)
	call(['restor', 'map=%s'%t_map, 'beam=%s'%t_beam, 'model=%s'%t_model, 'out=%s'%t_restor],stdout=logf,stderr=logf)
