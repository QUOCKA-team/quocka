...
def do_smooth(flist, blist, new_beam):
    ...
    for i, f in enumerate(tqdm(flist)):
        ...
        sm = bn+'.sm'
        jf = open('junk.txt', 'w')
        call(['fits', 'op=xyin', 'in=%s' %
              f, 'out=%s' % bn], stdout=jf, stderr=jf)
        call(['convol', 'map=%s' % bn, 'fwhm=%f' %
              new_beam, 'options=final', 'out=%s' % sm], stdout=jf, stderr=jf)
        call(['fits', 'op=xyout', 'in=%s' %
              sm, 'out=%s.fits' % sm], stdout=jf, stderr=jf)
        jf.close()
