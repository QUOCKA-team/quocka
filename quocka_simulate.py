...
class RealSource:
    ...
    def freq_subset(self, fmin=None, fmax=None, inplace=False):
        ...
        ind = np.where(np.logical_and(self.data['freq']>=fmin,self.data['freq']<=fmax))
        if inplace:
            for k in self.data.keys():
                self.data[k] = self.data[k][ind]
        else:
            subset = self.copy()
            for k in subset.data.keys():
...
class SimulatedSource:
    ...
    def __init__(self, template=None, freq=None, inoise=0., inoisestd=0., qnoise=0., qnoisestd=0., unoise=0., unoisestd=0.):
        ...
        if template is None and freq is None:
            ...
            assert type(template).__module__ == type(self).__module__, "Template must be a quocka_simulate object"
            self.data['freq'] = template.data['freq']
            self.data['lamsq'] = template.data['lamsq']
        else:
            assert type(freq).__module__ == np.__name__, "Frequencies must be provided as a numpy array"
    ...
    def add_simple_rm(self, pfrac, rm, chi0):
        ...
        self.model['QU']['QU%d'%(self.nmodels[1])] = ['simple', pfrac, rm, chi0]
        self.nmodels[1] += 1
    ...
    def apply_noise(self):
        for stokes in ['I','Q','U']:
            self.data[stokes+'obs'] = self.data[stokes] + self.noise[stokes]*np.random.standard_normal(self.data[stokes].shape)
            self.data[stokes+'err'] = self.noisestd[stokes]*np.random.standard_normal(self.data[stokes].shape) + self.noise[stokes]

    def plot_2x2(self, pltfile=None):
        plt.figure(figsize=(14,12))
        plt.subplot(221)
        plt.errorbar(self.data['freq'], self.data['Iobs'], yerr=self.data['Ierr'], marker='o', mfc='none', ls='none')
        plt.gca().set_yscale('log')
        plt.gca().set_xscale('log')
        plt.xlabel('Frequency (Hz)')
        plt.ylabel('Stokes I')
        plt.subplot(222)
        plt.errorbar(self.data['lamsq'], self.data['Qobs'], yerr=self.data['Qerr'], marker='o', mfc='none', ls='none')
        plt.errorbar(self.data['lamsq'], self.data['Uobs'], yerr=self.data['Uerr'], marker='o', mfc='none', ls='none')
        plt.legend(('Q','U'), numpoints=1, loc='best')
        plt.xlabel('Wavelength squared')
        plt.ylabel('Stokes Q,U')
        plt.subplot(223)
        plt.scatter(self.data['Qobs'], self.data['Uobs'], marker='o', c=self.data['lamsq'], s=32, cmap='rainbow', zorder=20)
        plt.errorbar(self.data['Qobs'], self.data['Uobs'], xerr=self.data['Qerr'], yerr=self.data['Uerr'], c='k', ls='none', zorder=10)
        ...
        plt.ylabel('Stokes U')
        plt.subplot(224)
        plt.plot(self.data['lamsq'],0.5*180./np.pi*np.arctan2(self.data['Uobs'],self.data['Qobs']),'ko')
        plt.xlabel('Wavelength squared')
