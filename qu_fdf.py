...
BFmodType = bfModTypeDict['./'+rootSname][archnewDesigStr]['BFmodType'].replace('sha','Sha')

para_file = np.genfromtxt(rootSname+'/chains/'+rootSname+'_arch_'+BFmodType+'__BFparams.txt', dtype=str, skip_header=3)
...
goodQUinds = np.where((np.abs(a[goodIInds,4])>0.3*np.nanmedian(np.abs(a[goodIInds,4]))) & (np.abs(a[goodIInds,6])>0.3*np.nanmedian(np.abs(a[goodIInds,6]))))[0]
...
def External_FDF(p,psi,RM,sigRM,delRM):
    phiVec = np.linspace(-2000,2000,4001) #Define phi
    polInit = np.zeros_like(phiVec) #create a vector of zeros
    polInit[np.where(phiVec==round(RM))] = p #Where the F-thin comp is, make it equal to the fractional polarisation
...
#     gaussianKernel = (sigRM*np.sqrt(2*np.pi))**-1 * np.exp(-0.5*(phiVec/sigRM)**2) #Kernel which smoothes the polarised emission due to foreground (Burn) depol
    ...
    topInit[np.where(np.abs(phiVec)<delRM/2)] = 1./delRM #Kernel which smoothes the pol due to slab / gradient depol
    topHatKernel = topInit

#     firstConv = np.convolve(polInit,gaussianKernel,mode='same') #Distribute the polarised emission over Faraday depth due to external foreground depol
    secondConvolve = np.convolve(polInit,topHatKernel,mode='same') #distribute the polarised emission due to slab depol
...
plt.plot(rmsyn_fdfclean[:,0], np.sqrt(rmsyn_fdfclean[:,1]**2+rmsyn_fdfclean[:,2]**2), label='RMclean FDF', color='tab:gray', linestyle='dotted', alpha=0.7)
rmsyn_comps = np.genfromtxt('./rmsyn/'+rootSname+'.reformat_FDFmodel.dat')
plt.plot(rmsyn_comps[:,0], np.sqrt(rmsyn_comps[:,1]**2+rmsyn_comps[:,2]**2), label='RMclean comps', linestyle='--', alpha=0.7)
...
for i in range(0,num_comps):
    if fit_comps[i][0] == 1:
        comp_vec = Rotation_FDF(fit_comps[i][1], fit_comps[i][2], fit_comps[i][3], fit_comps[i][4], fit_comps[i][5])
        comp_style = 'Rotation'
    elif fit_comps[i][0] == 2:
        comp_vec = External_FDF(fit_comps[i][1], fit_comps[i][2], fit_comps[i][3], fit_comps[i][4], fit_comps[i][5])
        comp_style = 'External'
    elif fit_comps[i][0] == 3:
        comp_vec = Internal_FDF(fit_comps[i][1], fit_comps[i][2], fit_comps[i][3], fit_comps[i][4], fit_comps[i][5])
        comp_style = 'Internal'
    elif fit_comps[i][0] == 4:
        comp_vec = Mixed_FDF(fit_comps[i][1], fit_comps[i][2], fit_comps[i][3], fit_comps[i][4], fit_comps[i][5])
        comp_style = 'Mixed'
    else: 
        continue
        
    plt.plot(phiVec, comp_vec*ISQ0, color='tab:orange', label='QUfit comps', alpha=0.7)
    title_str = title_str + 'QUfit_comp '+str(i+1)+': '+comp_style+', frac_pol='+"{:.2%}".format(fit_comps[i][1])+', PSI='+str(fit_comps[i][2])+', RM='+str(fit_comps[i][3])+', sigRM='+str(fit_comps[i][4])+', delRM='+str(fit_comps[i][5])+'\n'
