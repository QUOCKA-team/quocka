...
# 	imsize = 1600
	...
	call(['invert','vis=%s.%s'%(sourcename,freqband),
		...
		'imsize=%s'%(imsize),'cell=%s'%(cellsize),'robust=0.5','stokes=i',selstring,
	...
	call(['fits','in=%s'%(regridname),'op=xyout','out=%s.fits'%(regridname)], stdin=None, stdout=None, stderr=None, shell=False)
	call(['fits','in=%s'%(maskname),'op=xyout','out=%s.fits'%(maskname)], stdin=None, stdout=None, stderr=None, shell=False)
...
# #     call(['rm','-rf','%s.restor.%s.mfs.fits'%(sourcename,freqband)])
	...
	for i in range(1,2049,10):
		call(['invert','vis=%s.%s'%(sourcename,freqband),
			...
			',%s.d.%s.%04d.u'%(sourcename,freqband,i)+ ',%s.d.%s.%04d.v'%(sourcename,freqband,i),
			'beam=%s.beam.%s.%04d'%(sourcename,freqband,i),
			'imsize=%s'%(imsize),'cell=%s'%(cellsize),'robust=0.5','stokes=i,q,u,v',selstring,
