...
def writecube(data, beam, stoke, field, outdir, verbose=False):
    ...
    # Save the data
    fits.writeto(f"{outdir}/{outfile}", data["cube"], header=header, overwrite=True)
    if verbose:
...
def main(pool, args, verbose=False):
    ...
    for stoke in stokes:
        file_dict.update(
            {stoke: sorted(glob(f"{datadir}/{field}.*.{stoke}.cutout.bandcube.fits"))}
        )
    file_dict.update(
        {"freqs": sorted(glob(f"{datadir}/{field}.*.bandcube.frequencies.txt"))}
    )
    ...
    for stoke in stokes:
        ...
        for band in tqdm(bands, desc="Reading data", disable=(not verbose)):
            with fits.open(
                f"{datadir}/{field}.{band}.{stoke}.cutout.bandcube.fits",
                memmap=True,
                mode="denywrite",
            ) as hdulist:
                data = hdulist[0].data
                head = hdulist[0].header
                freq = np.loadtxt(f"{datadir}/{field}.{band}.bandcube.frequencies.txt")
                datadict.update(
                    {
                        band: {
                            "data": data,
        ...
        for band in tqdm(bands, desc="Regridding data", disable=(not verbose)):
            ...
            newcube[:] = out[:]
            datadict[band].update({"newdata": newcube})

        # Get scaling factors and convolution kernels
        for band in tqdm(
            bands, desc="Computing scaling factors", disable=(not verbose)
        ):
            ...
            datadict[band].update(
                {"conbeam": conbm, "fac": fac, "target header": target_header}
            )
            datadict.update({"target header": target_header})

        # Convolve data
        for band in tqdm(bands, desc="Smoothing data", disable=(not verbose)):
            smooth = partial(
                scipy.signal.convolve, in2=datadict[band]["conbeam"], mode="same"
            )
        ...
        stoke_dict.update({stoke: datadict})
        ...
        if args.debug:
            ...
            for band in bands:
                ...
                y = datadict[band]["fac"] * datadict[band]["smdata"][:, idx[0], idx[1]]
    ...
    for stoke in tqdm(stokes, desc="Making cubes", disable=(not verbose)):
        cube = np.vstack(
            [
                stoke_dict[stoke][band]["smdata"] * stoke_dict[stoke][band]["fac"]
                for band in bands
            ]
        )
        freq_cube = (
            np.concatenate([stoke_dict[stoke][band]["freq"] for band in bands]) * u.Hz
        )
        stoke_dict[stoke].update({"cube": cube, "freqs": freq_cube})
    ...
    if not args.dryrun:
        ...
        for stoke in tqdm(stokes, desc="Writing cubes", disable=(not verbose)):
            writecube(
                stoke_dict[stoke], new_beam, stoke, field, outdir, verbose=verbose
            )
