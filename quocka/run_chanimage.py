...
for vis in vislist:
    ...
    call(
        [
            "fits",
            "in=%s%s.%s.p2.fits" % (mfsdir, sourcename, freqband),
            "op=xyin",
    ...
    call(
        [
            "regrid",
            "in=%s.%s.p2.fits.map" % (sourcename, freqband),
            "out=%s" % (regridname),
            "tin=%s.d.%s.mfs.i" % (sourcename, freqband),
        ],
        stdin=None,
        stdout=None,
        stderr=None,
    ...
    call(
        [
            "maths",
            "exp=<%s>" % (regridname),
            "mask=<%s>.gt.%f" % (regridname, mask_level),
            "out=%s" % (maskname),
        ],
        stdin=None,
        stdout=None,
    ...
    call(
        ["fits", "in=%s" % (regridname), "op=xyout", "out=%s.fits" % (regridname)],
        stdin=None,
        stdout=None,
        stderr=None,
        shell=False,
    ...
    call(
        ["fits", "in=%s" % (maskname), "op=xyout", "out=%s.fits" % (maskname)],
        stdin=None,
        stdout=None,
        stderr=None,
        shell=False,
    ...
    for i in trange(1, 2049, 10):
        call(
            [
                "invert",
                "vis=%s.%s" % (sourcename, freqband),
                "map=%s.d.%s.%04d.i" % (sourcename, freqband, i)
                + ",%s.d.%s.%04d.q" % (sourcename, freqband, i)
        ...
        for stokes in ["i", "q", "u", "v"]:
            if not os.path.exists(
                "%s.d.%s.%04d.%s" % (sourcename, freqband, i, stokes)
            ):
                continue
            else:
                call(
                    [
                        "clean",
                        "map=%s.d.%s.%04d.%s" % (sourcename, freqband, i, stokes),
                        "beam=%s.beam.%s.%04d" % (sourcename, freqband, i),
                        "out=%s.model.%s.%04d.%s" % (sourcename, freqband, i, stokes),
                        "cutoff=%f" % (5.0 * imnoise),
                        "niters=1500",
                        "region=mask(%s)" % (maskname),
                    ],
                    stdin=None,
                    stdout=None,
                    stderr=None,
                ...
                call(
                    [
                        "fits",
                        "in=%s.%s.%04d.%s" % (sourcename, freqband, i, stokes),
                ...
                call(
                    ["rm", "-rf", "%s.d.%s.%04d.%s" % (sourcename, freqband, i, stokes)]
                )
                call(
                    [
                        "rm",
                        "-rf",
                        "%s.d.%s.%04d.%s.fits" % (sourcename, freqband, i, stokes),
                    ]
                )
                call(
                    [
                        "rm",
                        "-rf",
                        "%s.model.%s.%04d.%s" % (sourcename, freqband, i, stokes),
                    ]
                )
