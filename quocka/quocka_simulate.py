...
class RealSource:
    ...
    def read_data(self, filename, reformatted=False):
        ...
        if reformatted:
            ...
            self.data["freq"] = values[ind, 0] * 1.0e9
            self.data["lamsq"] = (C / (values[ind, 0] * 1.0e9)) ** 2
            self.data["I"] = values[ind, 1]
    ...
    def freq_subset(self, fmin=None, fmax=None, inplace=False):
        ...
        ind = np.where(
            np.logical_and(self.data["freq"] >= fmin, self.data["freq"] <= fmax)
        )
        if inplace:
...
class SimulatedSource:
    def __init__(
        ...
        if template is None and freq is None:
            print(
                "You must either provide a template or specify the frequencies to use"
            )
            raise ValueError
            ...
            assert (
                type(template).__module__ == type(self).__module__
            ), "Template must be a quocka_simulate object"
            self.data["freq"] = template.data["freq"]
            self.data["lamsq"] = template.data["lamsq"]
        else:
            assert (
                type(freq).__module__ == np.__name__
            ), "Frequencies must be provided as a numpy array"
    ...
    def __add__(self, y):
        ...
        if lm == 0:
            ...
            print("Add mode")
            # check frequencies
            if np.sum(np.abs(self.data["freq"] - y.data["freq"])) > 0.0:
    ...
    def write_data(self, filename, reformatted=False):
        if reformatted:
            ...
            datapack[:, 0] = self.data["freq"] / 1.0e9
            datapack[:, 1] = self.data["Iobs"]
    ...
    def add_simple_rm(self, pfrac, rm, chi0):
        ...
        pvals = pfrac * self.data["I"] * np.exp(2.0j * (chi0 + rm * self.data["lamsq"]))
    ...
    def add_dfr(self, pfrac, R, rm, chi0):
        ...
        pvals = (
            pfrac
            * self.data["I"]
            * np.sin(R * self.data["lamsq"])
            / (R * self.data["lamsq"])
            * np.exp(2.0j * (chi0 + rm * self.data["lamsq"]))
        )
        self.data["Q"] += np.real(pvals)
        self.data["U"] += np.imag(pvals)
    ...
    def add_ext(self, pfrac, sig, rm, chi0):
        ...
        pvals = (
            pfrac
            * self.data["I"]
            * np.exp(-2.0 * sig**2 * self.data["lamsq"] ** 2)
            * np.exp(2.0j * (chi0 + rm * self.data["lamsq"]))
        )
        self.data["Q"] += np.real(pvals)
        self.data["U"] += np.imag(pvals)
        ...
        for i in range(self.nmodels[1]):
            ...
            if qumodel[0] == "simple":
                ...
                if rm < np.min(phi) or rm > np.max(phi):
                    ...
                    model_fdf[ind] += pfrac * np.exp(1.0j * chi0)
            elif qumodel[0] == "dfr":
                # make a box spectrum
                # warn if out of range
                ...
                if rm > np.max(phi) or (rm + R) < np.min(phi):
                    ...
                    ind = np.where(
                        np.logical_and(phi <= (rm + 0.5 * R), phi >= rm - 0.5 * R)
                    )
                    model_fdf[ind] += pfrac * np.exp(1.0j * chi0)
            elif qumodel[0] == "ifd":
                ...
                if rm - srm > np.max(phi) or (rm + R + srm) < np.min(phi):
                    ...
                    if rm > np.min(phi):
                        ind = np.where(phi < rm)
                        amp = pfrac * np.exp(-((phi[ind] - rm) ** 2) / (2.0 * srm**2))
                        model_fdf[ind] += amp * np.exp(1.0j * chi0)
                    if np.max(phi) > (rm + R):
                        ind = np.where(phi > (rm + R))
                        amp = pfrac * np.exp(
                            -((phi[ind] - (rm + R)) ** 2) / (2.0 * srm**2)
                        )
                        model_fdf[ind] += amp * np.exp(1.0j * chi0)
            else:
    ...
    def apply_noise(self):
        for stokes in ["I", "Q", "U"]:
            self.data[stokes + "obs"] = self.data[stokes] + self.noise[
                stokes
            ] * np.random.standard_normal(self.data[stokes].shape)
            self.data[stokes + "err"] = (
                self.noisestd[stokes]
                * np.random.standard_normal(self.data[stokes].shape)
                + self.noise[stokes]
            )
    ...
    def plot_2x2(self, pltfile=None):
        ...
        plt.plot(
            self.data["lamsq"],
            0.5 * 180.0 / np.pi * np.arctan2(self.data["Uobs"], self.data["Qobs"]),
            "ko",
        )
        plt.xlabel("Wavelength squared")
