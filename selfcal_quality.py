...
for t in vislist:
	...
	cutout = Cutout2D(hdu[0].data[0,0], position=(centre_pix_x,centre_pix_y), size=(400,400), wcs=wcs)

	ax = fig.add_subplot(2,2,1)
	plt.imshow(cutout.data,vmin=-10.0*sigma_p2, vmax=30.0*sigma_p2, origin='lower', cmap='cubehelix')
	plt.contour(cutout.data, levels=[mask_p0])
	plt.xticks([])
	plt.yticks([])
	ax.set_title('mask level %s sigma \n rms %s'%(int(mask_p0/sigma_p0), sigma_p0))
	hdu.close()
	...
	plt.xticks([])
	plt.yticks([])
	ax.set_title('mask level %s sigma \n rms %s'%(int(mask_p1/sigma_p1), sigma_p1))
	hdu.close()
	...
	plt.xticks([])
	plt.yticks([])
	ax.set_title('mask level %s sigma \n rms %s'%(int(mask_p2/sigma_p2), sigma_p2))
	hdu.close()
	...
	if sigma_p0>sigma_p1>sigma_p2:
		plt.suptitle('%s: peak flux %s, min flux %s'%(t,peak_max_p0,peak_min_p0), color='tab:green', fontsize=15)
		plt.savefig('%s_0_scal_qua.png'%t, dpi=300, bbox_inches='tight')
	else:
		plt.suptitle('%s: peak flux %s, min flux %s'%(t,peak_max_p0,peak_min_p0), color='tab:red', fontsize=15)
